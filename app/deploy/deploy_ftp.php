<?php
// deploy_ftp.php (com verificação real de diretório e força criação via ftp_mkdir)
...
logDeploy("[VALIDAÇÃO] Buscando diretórios em: " . dirname($ftp_repo_path));
$dir_parent = dirname($ftp_repo_path);
$dir_name = basename($ftp_repo_path);
$existing_dirs = ftp_nlist($conn, $dir_parent);

if (!$existing_dirs || !in_array("$dir_parent/$dir_name", $existing_dirs)) {
    logDeploy("[FORÇA] mkdir manual: $ftp_repo_path");
