...
function initializeRepository($repo_name)
{
    ...
    if ($repo_check['status'] == 404 || empty($repo_check['response'])) {
        return createFile($repo_name, 'README.md', 'Este é um repositório inicializado automaticamente pelo DevBotAI.', 'Primeiro commit do repositório.');
    }
...
function createRepository($repo_name, $private = true)
{
    ...
    if ($response['status'] == 201) {
        ...
        if (!checkBranchExists($repo_name)) {
            ...
            createFile($repo_name, 'README.md', 'Este é um repositório inicializado automaticamente pelo DevBotAI.', 'Primeiro commit do repositório.');
        }
    }
